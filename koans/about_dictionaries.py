...
class AboutDictionaries(Koan):
    ...
    # Added test
    def test_making_a_dictionary_from_a_sequence_of_keys_range_with_custom_list(self):
        cus_list = [4, 2]
        nums = {}.fromkeys(range(5), cus_list)
        self.assertEqual(5, len(nums))
        self.assertEqual([4, 2], nums[0])
        self.assertEqual([4, 2], nums[4])
